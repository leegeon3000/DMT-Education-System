...
const TeacherLayout: React.FC = () => {
  ...
  const navigation = [
    ...
    },
    {
      ...
      icon: Users,
      current: location.pathname.includes('/teacher/classes')
    },
    {
      name: 'Lịch dạy',
      href: '/teacher/calendar',
      icon: Calendar,
      current: location.pathname.includes('/teacher/calendar')
    },
    {
      name: 'Bài tập',
      href: '/teacher/assignments',
      icon: FileText,
      current: location.pathname.includes('/teacher/assignments')
    },
    {
      name: 'Chấm điểm',
      href: '/teacher/grading',
      icon: ClipboardList,
      current: location.pathname.includes('/teacher/grading')
    },
    {
      name: 'Điểm danh',
      href: '/teacher/attendance',
      icon: UserCheck,
      current: location.pathname.includes('/teacher/attendance')
    },
    {
      name: 'Tài liệu',
      href: '/teacher/materials',
      icon: BookOpen,
      current: location.pathname.includes('/teacher/materials')
    },
    {
      name: 'Khảo sát',
      href: '/teacher/surveys',
      icon: BarChart3,
      current: location.pathname.includes('/teacher/surveys')
    },
    {
      name: 'Báo cáo',
      href: '/teacher/reports',
      icon: TrendingUp,
      current: location.pathname.includes('/teacher/reports')
    },
    {
      name: 'Chấm công',
      href: '/teacher/timesheet',
      icon: Clock,
      current: location.pathname.includes('/teacher/timesheet')
    }
  ];
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-gradient-to-b from-slate-900 to-slate-800 shadow-2xl transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${
        ...
        <div className="flex items-center justify-between h-20 px-6 border-b border-slate-700/50">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <span className="text-xl font-bold text-white">DMT Teacher</span>
          ...
          <div className="relative">
            ...
            >
              <div className="flex items-center flex-1">
                <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-blue-600 rounded-full flex items-center justify-center shadow-lg ring-2 ring-slate-700">
                  <GraduationCap className="w-5 h-5 text-white" />
                </div>
                <div className="ml-3 text-left">
                  <p className="text-sm font-semibold text-white">{user?.name || 'Teacher'}</p>
                  <p className="text-xs text-slate-400">{user?.role || 'Giáo viên'}</p>
                </div>
              </div>
          ...
          <div className="flex items-center justify-between h-20 px-6">
            ...
            <div className="flex-1 flex items-center justify-end space-x-4">
              ...
              <div className="hidden md:block relative">
                <input
                  type="text"
                  placeholder="Tìm kiếm học viên, lớp học, bài tập..."
              ...
              <div className="hidden md:flex items-center space-x-3 pl-4 border-l border-gray-200">
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-500">{user?.role || 'Giáo viên'}</p>
                </div>
